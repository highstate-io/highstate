...
async function tryRunEditorAction(editor: monaco.editor.IStandaloneCodeEditor, id: string) {
  try {
...
/**
 * Folds inclusion/secret blocks using folding ranges marked as `Comment` kind.
 *
 * It prefers `Fold All Block Comments` so only our explicit ranges are folded.
 */
export async function foldEntityYamlByDefault(
  editor: monaco.editor.IStandaloneCodeEditor,
  regions: FoldingRegion[],
): Promise<void> {
  const hasAnyRegions = regions.some(region => region.end > region.start)
  if (!hasAnyRegions) {
    return
  }

  const foldedBlockComments = await tryRunEditorAction(editor, "editor.foldAllBlockComments")
  if (foldedBlockComments) {
    return
  }

  await tryRunEditorAction(editor, "editor.foldAllRegions")
}
