...
import { backendRoleRulesSchema } from "@highstate/backend/shared"
...
const { apiKey, serviceAccounts, restrictionOptions, loading } = defineProps<{
  apiKey?: BackendApiKeyOutput | null
  serviceAccounts: BackendApiKeyServiceAccountOption[]
  restrictionOptions: BackendRoleRestrictionOptions
  loading?: boolean
}>()
const visible = defineModel<boolean>("visible", { required: true })
const emit = defineEmits<{ save: [input: BackendApiKeyInput] }>()
const title = ref("")
const description = ref("")
const serviceAccountId = ref<string>()
const expiresAt = ref("")
const restrictionRules = ref<BackendRoleRules>([])
...
const restrictionSchemaValid = computed(
  () =>
    ...
    backendRoleRulesSchema.safeParse(restrictionRules.value).success,
)
...
function restrictionsCover(allowedRule: BackendRoleRule, rule: BackendRoleRule): boolean {
...
function restrictionIds(restriction: BackendPermissionRestriction): string[] {
...
<template>
  <VDialog v-model="visible" max-width="1200" class="api-key-dialog">
    <VCard :title="apiKey ? 'Edit backend API key' : 'Create backend API key'">
