...
const { serviceAccount, loading } = defineProps<{
  serviceAccount?: ServiceAccountOutput | null
  loading?: boolean
}>()
const visible = defineModel<boolean>("visible", { required: true })
const emit = defineEmits<{ save: [input: ServiceAccountInput] }>()
const title = ref("")
...
<template>
  <VDialog v-model="visible" max-width="620">
    <VCard
      :title="serviceAccount ? 'Edit project service account' : 'Create project service account'"
      color="#2d2d2d"
    >
