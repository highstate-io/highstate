...
import { etcd, k8s } from "@highstate/library"
...
const clientEndpoints = endpoints.filter(e => e.port === 2379)

export default outputs({
  connection: makeEntityOutput({
    identity: args.appName,
  ...
  $statusFields: {
    endpoints: clientEndpoints.map(l4EndpointToString),
  },

  $triggers: [backupJobPair?.handleTrigger(invokedTriggers)],
  $terminals: [backupJobPair?.terminal],
})
