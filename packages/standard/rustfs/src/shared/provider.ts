...
export async function getProvider(connection: Input<rustfs.Connection>): Promise<Provider> {
  ...
  return await getOrCreate(providers, getEntityId(resolvedConnection), async entityId => {
    ...
    return new Provider(entityId, {
      ...
      minioUser: resolvedConnection.credentials.accessKey,
      minioPassword: resolvedConnection.credentials.secretKey.value,
