# Authoring the Handbook

The handbook preserves current knowledge shared across the Highstate workspace.
...
Each fact has exactly one owning handbook document.
...
The handbook owns durable system boundaries, dependency direction, and compatibility constraints.
...
- `authoring/` for rules governing this handbook

Do not create empty, miscellaneous, or `meta` sections.
Use a section `README.md` for orientation and links; keep the root `README.md` limited to the handbook's
...
Write non-authoring sections for their users, not handbook maintainers.
...
Enter `devenv shell`, then run `bunx --bun nx run handbook:check` and
`bunx --bun nx run handbook:links`.
Use `bunx --bun nx run handbook:format` to format Markdown.
...
- Confirm that the content belongs in the handbook rather than with one package or source file.
