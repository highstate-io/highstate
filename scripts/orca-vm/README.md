...
The root [`orca.yaml`](../../orca.yaml) invokes each provider lifecycle script directly, making each script
responsible for Orca's input and output contract.
...
Add direct lifecycle commands to root `orca.yaml` and include the provider scripts in the Orca VM Nx
check.
